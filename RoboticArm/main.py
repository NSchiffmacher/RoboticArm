...
class Scene(BaseScene):
    ...
    def update(self, dt, events):
        ...
        if self.main_focus and imgui.is_mouse_clicked(imgui.MOUSE_BUTTON_MIDDLE):
            if self.add_obstacle:
                mouse_pos = self.draw_frame_to_global_frame(V(*imgui.get_mouse_pos()))
                removed = self.obstacles.remove_point_from_current(mouse_pos, 2)
    ...
    def draw(self, fenetre):
        ...
        with imgui.begin("main", False, imgui.WINDOW_NO_RESIZE | imgui.WINDOW_NO_COLLAPSE | imgui.WINDOW_MENU_BAR | imgui.WINDOW_NO_MOVE | imgui.WINDOW_NO_BRING_TO_FRONT_ON_FOCUS):
            # menu bar
                ...
                if obstacles_menu_bar.opened:
                    ...
                    with imgui.begin_menu('Obstacles') as file_menu:
                        if file_menu.opened:
                            ...
                            if clicked:
                                def_path = Path(os.getcwd()) / self.options.obstacles_path / self.options.default_obstacles
    ...
    def add_obstacle_imgui(self):
        with imgui.begin("Add obstacle", flags=imgui.WINDOW_NO_FOCUS_ON_APPEARING | imgui.WINDOW_NO_RESIZE):
